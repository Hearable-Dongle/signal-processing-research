...
def main():
    ...
    output_dir = Path("simulation/simulations/configs/library_scene")
    ...
    for k in speakers_counts:
        for i in range(scenes_per_count):
            # Room Definition (Library)
            # Mimics small restaurant size: 5m - 10m
            room_dims = [
            ...
            # Library Acoustics:
            # "not seem quite as echo-y" -> Higher absorption than restaurant (which was 0.2)
            # Bookshelves, carpet, acoustic tiles imply absorption around 0.4 - 0.6
            absorption = round(random.uniform(0.4, 0.6), 2)
            
            room = Room(dimensions=room_dims, absorption=absorption)
            ...
            # Lower gain for background noise
            noise_gain = round(random.uniform(0.05, 0.2), 2)
            ...
            filename = f"library_k{k}_scene{i:02d}.json"
