...
def _stage_configs(stage_name: str, parents: list[TrialResult], baseline_lsd: float) -> list[dict[str, Any]]:
    ...
    if stage_name == "stage1":
        for parent in parents:
            for notch_freq in (480.0, 500.0, 520.0):
...
def _shortlist_payload(results: list[TrialResult], baseline: TrialResult) -> list[dict[str, Any]]:
    valid = [result for result in results if bool(result.summary["valid"])]
    valid_sorted = sorted(valid, key=lambda result: _ranking_key_with_hum(result.summary))
    shortlist: list[TrialResult] = [baseline]
    shortlist.extend(valid_sorted[:3])
    if valid_sorted:
        shortlist.append(min(valid_sorted, key=lambda result: float(result.summary["artifact_500hz_cluster_ratio_mean"])))
        shortlist.append(min(valid_sorted, key=lambda result: float(result.summary["artifact_hf_flux_p95_mean"])))
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tune RNNoise artifact cleanup on nano symposium behind-poster.")
    parser.add_argument(
        "--input-path",
        default="data-collection/nano-sympoium/nano-symposium-behind-poster",
    )
    parser.add_argument(
        "--out-root",
        default="beamforming/benchmark/rnnoise_artifact_tuning_behind_poster",
    )
...
def main() -> None:
    ...
    try:
        ...
        stage1_top = sorted([r for r in stage1_results if bool(r.summary["valid"])], key=lambda r: _ranking_key_with_hum(r.summary))[:3]
        if not stage1_top:
            stage1_top = sorted(stage1_results, key=lambda r: _ranking_key_with_hum(r.summary))[:3]
        ...
        stage2_top = sorted([r for r in stage2_results if bool(r.summary["valid"])], key=lambda r: _ranking_key_with_hum(r.summary))[:2]
        if not stage2_top:
            stage2_top = sorted(stage2_results, key=lambda r: _ranking_key_with_hum(r.summary))[:2]
