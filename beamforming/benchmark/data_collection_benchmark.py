...
def _run_recording_method_job(
    *,
    ...
    postfilter_noise_source: str,
    postfilter_input_source: str,
    ...
    beamformer_rnn_skip_refresh_when_clean: bool,
    beamformer_rnn_dirty_threshold: float,
    beamformer_rnn_dirty_eps: float,
    beamformer_rnn_dirty_stat: str,
    beamformer_sparse_solve_enabled: bool,
    beamformer_sparse_solve_stride: int,
    beamformer_sparse_solve_min_freq_hz: float,
    beamformer_sparse_solve_interp: str,
    beamformer_weight_reuse_enabled: bool,
    beamformer_weight_smoothing_alpha: float,
    beamformer_doa_refresh_tolerance_deg: float,
    ...
    req = SessionStartRequest(
        ...
        fast_path={
            ...
            "postfilter_noise_source": str(postfilter_noise_source),
            ...
            "beamformer_rnn_skip_refresh_when_clean": bool(beamformer_rnn_skip_refresh_when_clean),
            "beamformer_rnn_dirty_threshold": float(beamformer_rnn_dirty_threshold),
            "beamformer_rnn_dirty_eps": float(beamformer_rnn_dirty_eps),
            "beamformer_rnn_dirty_stat": str(beamformer_rnn_dirty_stat),
            "beamformer_sparse_solve_enabled": bool(beamformer_sparse_solve_enabled),
            "beamformer_sparse_solve_stride": int(beamformer_sparse_solve_stride),
            "beamformer_sparse_solve_min_freq_hz": float(beamformer_sparse_solve_min_freq_hz),
            "beamformer_sparse_solve_interp": str(beamformer_sparse_solve_interp),
            "beamformer_weight_reuse_enabled": bool(beamformer_weight_reuse_enabled),
            "beamformer_weight_smoothing_alpha": float(beamformer_weight_smoothing_alpha),
            "beamformer_doa_refresh_tolerance_deg": float(beamformer_doa_refresh_tolerance_deg),
    ...
    row = {
        ...
        "beamformer_rnn_skip_refresh_when_clean": bool(beamformer_rnn_skip_refresh_when_clean) if is_mvdr else False,
        "beamformer_rnn_dirty_threshold": float(beamformer_rnn_dirty_threshold) if is_mvdr else float("nan"),
        "beamformer_rnn_dirty_eps": float(beamformer_rnn_dirty_eps) if is_mvdr else float("nan"),
        "beamformer_rnn_dirty_stat": str(beamformer_rnn_dirty_stat) if is_mvdr else "",
        "beamformer_sparse_solve_enabled": bool(beamformer_sparse_solve_enabled) if is_mvdr else False,
        "beamformer_sparse_solve_stride": int(beamformer_sparse_solve_stride) if is_mvdr else 0,
        "beamformer_sparse_solve_min_freq_hz": float(beamformer_sparse_solve_min_freq_hz) if is_mvdr else float("nan"),
        "beamformer_sparse_solve_interp": str(beamformer_sparse_solve_interp) if is_mvdr else "",
        "beamformer_weight_reuse_enabled": bool(beamformer_weight_reuse_enabled) if is_mvdr else False,
        "beamformer_weight_smoothing_alpha": float(beamformer_weight_smoothing_alpha) if is_mvdr else float("nan"),
        "beamformer_doa_refresh_tolerance_deg": float(beamformer_doa_refresh_tolerance_deg) if is_mvdr else float("nan"),
        ...
        "beamforming_rtf": float(summary.get("beamforming_rtf", float("nan"))),
        "postfilter_rtf": float(summary.get("postfilter_rtf", float("nan"))),
        "pipeline_rtf": float(summary.get("pipeline_rtf", float("nan"))),
        "fast_avg_ms": float(summary["fast_avg_ms"]),
        "slow_avg_ms": float(summary["slow_avg_ms"]),
        "beamforming_avg_ms": float(summary.get("beamforming_avg_ms", float("nan"))),
        "postfilter_avg_ms": float(summary.get("postfilter_avg_ms", float("nan"))),
        "pipeline_avg_ms": float(summary.get("pipeline_avg_ms", float("nan"))),
        "fast_srp_avg_ms": float(summary.get("fast_stage_avg_ms", {}).get("srp", float("nan"))),
        "fast_beamform_avg_ms": float(summary.get("fast_stage_avg_ms", {}).get("beamform", float("nan"))),
        ...
        "beamformer_weights_reused_frames": float(summary.get("beamformer_weights_reused_frames", float("nan"))),
        "beamformer_dirty_stat_last": float(summary.get("beamformer_dirty_stat_last", float("nan"))),
        "beamformer_dirty_stat_mean": float(summary.get("beamformer_dirty_stat_mean", float("nan"))),
        "beamformer_dirty_stat_p95": float(summary.get("beamformer_dirty_stat_p95", float("nan"))),
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--postfilter-noise-source", choices=["tracked_mono", "beamformer_rnn_output"], default="tracked_mono")
    parser.add_argument("--postfilter-input-source", choices=["beamformed_mono", "raw_mix_mono"], default="beamformed_mono")
    ...
    parser.add_argument("--beamformer-rnn-skip-refresh-when-clean", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--beamformer-rnn-dirty-threshold", type=float, default=0.0)
    parser.add_argument("--beamformer-rnn-dirty-eps", type=float, default=1e-8)
    parser.add_argument("--beamformer-rnn-dirty-stat", choices=["max", "mean"], default="max")
    parser.add_argument("--beamformer-sparse-solve-enabled", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--beamformer-sparse-solve-stride", type=int, default=1)
    parser.add_argument("--beamformer-sparse-solve-min-freq-hz", type=float, default=200.0)
    parser.add_argument("--beamformer-sparse-solve-interp", choices=["linear_complex"], default="linear_complex")
    parser.add_argument("--beamformer-weight-reuse-enabled", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--beamformer-weight-smoothing-alpha", type=float, default=1.0)
    parser.add_argument("--beamformer-doa-refresh-tolerance-deg", type=float, default=5.0)
    ...
    try:
        ...
        def _submit_job(job: tuple[int, int, str, Path, str]) -> dict:
            ...
            return _run_recording_method_job(
                ...
                postfilter_noise_source=str(args.postfilter_noise_source),
                postfilter_input_source=str(args.postfilter_input_source),
                ...
                beamformer_rnn_skip_refresh_when_clean=bool(args.beamformer_rnn_skip_refresh_when_clean),
                beamformer_rnn_dirty_threshold=float(args.beamformer_rnn_dirty_threshold),
                beamformer_rnn_dirty_eps=float(args.beamformer_rnn_dirty_eps),
                beamformer_rnn_dirty_stat=str(args.beamformer_rnn_dirty_stat),
                beamformer_sparse_solve_enabled=bool(args.beamformer_sparse_solve_enabled),
                beamformer_sparse_solve_stride=int(args.beamformer_sparse_solve_stride),
                beamformer_sparse_solve_min_freq_hz=float(args.beamformer_sparse_solve_min_freq_hz),
                beamformer_sparse_solve_interp=str(args.beamformer_sparse_solve_interp),
                beamformer_weight_reuse_enabled=bool(args.beamformer_weight_reuse_enabled),
                beamformer_weight_smoothing_alpha=float(args.beamformer_weight_smoothing_alpha),
                beamformer_doa_refresh_tolerance_deg=float(args.beamformer_doa_refresh_tolerance_deg),
