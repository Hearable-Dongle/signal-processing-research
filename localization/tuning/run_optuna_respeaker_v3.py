...
DEFAULT_OUT_ROOT = Path("localization/tuning/results/respeaker_v3_optuna")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Optuna localization tuning for ReSpeaker v3 on testing_specific_angles.")
...
def _get_optuna():
    try:
        ...
        raise RuntimeError(
            "optuna is not installed in the active environment. Run this from the beamforming/localization environment that includes optuna."
        ) from exc
    return optuna
...
def _default_trial_params(method: str, cfg: dict[str, Any], fs: int) -> dict[str, Any]:
    window_ms = int(round(1000.0 * int(cfg.get("nfft", 512)) / float(fs)))
    window_ms = max(10, min(250, int(5 * round(window_ms / 5.0))))
    params = {
        "window_ms": window_ms,
...
def _suggest_eval_config(optuna, trial, method: str, base_cfg: dict[str, Any], fs: int) -> dict[str, Any]:
    window_ms = trial.suggest_int("window_ms", 10, 250, step=5)
    overlap = trial.suggest_float("overlap", 0.25, 0.90, step=0.05)
    freq_low_hz = trial.suggest_int("freq_low_hz", 100, 1200, step=50)
    ...
    trial.set_user_attr("nfft", cfg["nfft"])
    return cfg


def _objective_factory(method: str, base_cfg: dict[str, Any], subset_scenes, profile: str, method_workers: int):
    optuna = _get_optuna()
    fs = 16000
    ...
    def objective(trial):
        ...
        trial.set_user_attr("subset_n_scenes", aggregate["n_scenes"])
...
def _run_worker_loop(method: str, study_name: str, storage_url: str, deadline_ts: float, objective_args: dict[str, Any]) -> dict[str, Any]:
    ...
    objective = _objective_factory(
        ...
        method_workers=int(objective_args["eval_workers"]),
    )
...
def _full_eval_trial(method: str, trial_summary: dict[str, Any], base_cfg: dict[str, Any], full_scenes, profile: str, eval_workers: int) -> dict[str, Any]:
    params = dict(trial_summary["params"])
    method_cfg = build_eval_config(
        base_cfg=base_cfg,
        fs=16000,
...
def main() -> None:
    ...
    for method in methods:
        ...
        study_name = f"respeaker_v3_{method.lower().replace('-', '_')}"
        default_params = _default_trial_params(method, base_cfgs[method], fs=16000)
    ...
    for method in methods:
            ...
            export_csv_rows.append(
                {
                    ...
                    "nfft": trial.user_attrs.get("nfft"),
                    ...
                    "mae_deg_matched_mean": trial.user_attrs.get("mae_deg_matched_mean"),
                    "acc_within_10deg_mean": trial.user_attrs.get("acc_within_10deg_mean"),
                    "recall_mean": trial.user_attrs.get("recall_mean"),
                    "misses_mean": trial.user_attrs.get("misses_mean"),
                    "false_alarms_mean": trial.user_attrs.get("false_alarms_mean"),
                    "angle_mae_std": trial.user_attrs.get("angle_mae_std"),
                    "layout_mae_std": trial.user_attrs.get("layout_mae_std"),
                    "runtime_seconds_mean": trial.user_attrs.get("runtime_seconds_mean"),
                }
            )
        ...
        for trial_summary in trial_rows:
            full_eval_rows.append(
                _full_eval_trial(
                    ...
                    full_scenes=scenes,
                    profile=args.profile,
                    eval_workers=max(1, min(4, method_workers[method])),
                )
            )
        full_eval_rows = sorted(full_eval_rows, key=lambda row: (-float(row["balanced_score"]), float(row["mae_deg_matched_mean"] or 1e9)))
        write_json(info["method_dir"] / "best_full_eval.json", full_eval_rows)
        write_csv(
            ...
            [
                {
                    ...
                    "mae_deg_matched_mean": row["mae_deg_matched_mean"],
                    "acc_within_10deg_mean": row["acc_within_10deg_mean"],
                    "recall_mean": row["recall_mean"],
                    "misses_mean": row["misses_mean"],
                    "false_alarms_mean": row["false_alarms_mean"],
                    "runtime_seconds_mean": row["runtime_seconds_mean"],
                    "angle_mae_std": row["angle_mae_std"],
                    "layout_mae_std": row["layout_mae_std"],
        ...
        if best_row is not None:
            best_payload = {
                ...
                "mae_deg_matched_mean": best_row["mae_deg_matched_mean"],
                "acc_within_10deg_mean": best_row["acc_within_10deg_mean"],
                "recall_mean": best_row["recall_mean"],
                "misses_mean": best_row["misses_mean"],
                "false_alarms_mean": best_row["false_alarms_mean"],
                "runtime_seconds_mean": best_row["runtime_seconds_mean"],
                "angle_mae_std": best_row["angle_mae_std"],
                "layout_mae_std": best_row["layout_mae_std"],
    ...
    overall_best_rows = sorted(overall_best_rows, key=lambda row: (-float(row["balanced_score"]), float(row["mae_deg_matched_mean"] or 1e9), row["method"]))
    ...
    print(f"Wrote Optuna tuning outputs to {out_dir}")
