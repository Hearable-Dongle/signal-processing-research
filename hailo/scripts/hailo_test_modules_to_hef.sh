...
# 1 = add --quick_opt during HEF compile (faster iteration), 0 = full optimize path.
...
# Optional comma-separated case filter, e.g.
# MODULE_FILTER="norm_affine,encoder,hailo_convtasnet_k1"
MODULE_FILTER="${MODULE_FILTER:-}"

SUMMARY="${RUN_DIR}/module_hef_summary.tsv"
if [[ ! -f "$SUMMARY" ]]; then
...
in_filter() {
  ...
  IFS=',' read -r -a arr <<< "$MODULE_FILTER"
  local x
  for x in "${arr[@]}"; do
    if [[ "$x" == "$tag" ]]; then
      return 0
...
run_case() {
  ...
  if ! in_filter "$tag"; then
    echo "[SKIP] ${tag} (not in MODULE_FILTER)"
    return 0
  fi
  ...
  if [[ "$har_success" == "true" && "$hef_success" == "true" ]]; then
    ...
    echo "[HEF FAIL] ${tag} (har ok, hef fail)"
  else
    echo "[HAR FAIL] ${tag}"
  fi
...
# Stage1 primitives
run_case "stage1_primitives" "norm" "norm_affine" --in_chan 128 --norm_mode affine --time_len 1024
run_case "stage1_primitives" "norm" "norm_identity" --in_chan 128 --norm_mode identity --time_len 1024
run_case "stage1_primitives" "activation" "activation_sigmoid" --in_chan 128 --mask_act sigmoid --time_len 1024

# Stage2 block
run_case "stage2_block" "conv1d_block" "conv1d_block_skip0" --in_chan 128 --hid_chan 256 --skip_chan 0 --kernel_size 3 --dilation 1 --time_len 1024

# Stage3 tdconvnet (single minimal representative)
run_case "stage3_tdconvnet" "tdconvnet" "tdconvnet_k1" --in_chan 128 --out_chan 128 --n_src 2 --bn_chan 128 --hid_chan 256 --skip_chan 128 --n_blocks 1 --n_repeats 1 --truncate_k_blocks 1 --time_len 1024 --mask_act sigmoid

# Stage4 enc/dec
run_case "stage4_encdec" "encoder" "encoder" --n_filters 256 --encdec_kernel_size 16 --encdec_stride 8 --time_len 16000
run_case "stage4_encdec" "decoder" "decoder_conv1x1" --in_chan 256 --n_src 2 --time_len 2000

# Stage5 smallest full-topology representative module
run_case "stage5_full_k1" "hailo_convtasnet" "hailo_convtasnet_k1" --n_src 2 --n_filters 256 --bn_chan 128 --hid_chan 256 --skip_chan 128 --n_blocks 1 --n_repeats 1 --truncate_k_blocks 1 --mask_mul_mode bypass --skip_topology_mode project --decoder_mode conv1x1_head --time_len 16000

echo "[DONE] Module HEF test run_ts=${RUN_TS}"
