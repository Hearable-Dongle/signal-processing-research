...
# 1) temporal sweeps on existing micro modules
for L in 2000 1024 512 256 128; do
  run_case "stage1_temporal" "convtas_source_projector_out0" "allocfix_temporal_source_out0_w${L}" "$L" 256 \
    "${COMMON_ARGS[@]}" --time_len $((L * 8))
  run_case "stage1_temporal" "convtas_decoder_head_src0" "allocfix_temporal_dec_head0_w${L}" "$L" 512 \
    "${COMMON_ARGS[@]}" --time_len $((L * 8))
done

# 2) ultra-micro blockwise conv decomposition smoke
run_case "stage2_blocks" "convtas_source_projector_block" "allocfix_block_source_s0_o0_i0_w256" 256 64 \
  "${COMMON_ARGS[@]}" --block_chan 64 --proj_src_idx 0 --out_block_idx 0 --in_block_idx 0 --time_len 2048
run_case "stage2_blocks" "convtas_source_projector_block" "allocfix_block_source_s0_o0_i1_w256" 256 64 \
  "${COMMON_ARGS[@]}" --block_chan 64 --proj_src_idx 0 --out_block_idx 0 --in_block_idx 1 --time_len 2048
run_case "stage2_blocks" "convtas_decoder_pre_block" "allocfix_block_decpre_h0_o0_i0_w256" 256 64 \
  "${COMMON_ARGS[@]}" --block_chan 64 --half_idx 0 --out_block_idx 0 --in_block_idx 0 --time_len 2048
run_case "stage2_blocks" "convtas_decoder_pre_block" "allocfix_block_decpre_h0_o0_i1_w256" 256 64 \
  "${COMMON_ARGS[@]}" --block_chan 64 --half_idx 0 --out_block_idx 0 --in_block_idx 1 --time_len 2048
run_case "stage2_blocks" "convtas_decoder_head_block" "allocfix_block_dechead_s0_i0_w256" 256 64 \
  "${COMMON_ARGS[@]}" --block_chan 64 --head_src_idx 0 --in_block_idx 0 --time_len 2048
run_case "stage2_blocks" "convtas_decoder_head_block" "allocfix_block_dechead_s0_i1_w256" 256 64 \
  "${COMMON_ARGS[@]}" --block_chan 64 --head_src_idx 0 --in_block_idx 1 --time_len 2048

if [[ "$PROFILE" == "full" ]]; then
  for src in 0 1; do
    for ob in 0 1 2 3; do
      for ib in 0 1 2 3; do
        run_case "stage2_blocks_full" "convtas_source_projector_block" "allocfix_full_source_s${src}_o${ob}_i${ib}_w256" 256 64 \
          "${COMMON_ARGS[@]}" --block_chan 64 --proj_src_idx "$src" --out_block_idx "$ob" --in_block_idx "$ib" --time_len 2048
      done
    done
  done
fi

echo "[DONE] Allocator mapping fixes run_ts=${RUN_TS}"
