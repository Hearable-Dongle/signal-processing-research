...
SUMMARY="${RUN_DIR}/fine_submodule_hef_calib_match_summary.tsv"
if [[ ! -f "$SUMMARY" ]]; then
...
run_case() {
  ...
  echo "[CASE] ${tag} (input_len=${input_len}, input_chan=${input_chan})"
...
COMMON_ARGS=(--n_src 2 --n_filters 256 --bn_chan 128 --hid_chan 256 --skip_chan 128 --n_blocks 1 --n_repeats 1 --truncate_k_blocks 1 --mask_mul_mode bypass --skip_topology_mode project --decoder_mode conv1x1_head)

run_case "stage_fine_encoder" "encoder_conv_only" "calib_match_fine_encoder_conv_only" 16000 1 \
  --n_filters 256 --encdec_kernel_size 16 --encdec_stride 8 --time_len 16000

run_case "stage_fine_full" "convtas_encoder_only" "calib_match_fine_full_encoder_only" 16000 1 \
  "${COMMON_ARGS[@]}" --time_len 16000

run_case "stage_fine_full" "convtas_masker_only" "calib_match_fine_full_masker_only" 2000 256 \
  "${COMMON_ARGS[@]}" --time_len 16000

run_case "stage_fine_masker" "convtas_masker_bottleneck_only" "calib_match_fine_full_masker_bottleneck_only" 2000 256 \
  "${COMMON_ARGS[@]}" --time_len 16000

run_case "stage_fine_masker" "convtas_masker_tcn_block0_only" "calib_match_fine_full_masker_tcn_block0_only" 2000 128 \
  "${COMMON_ARGS[@]}" --time_len 16000

run_case "stage_fine_masker" "convtas_masker_mask_head_only" "calib_match_fine_full_masker_mask_head_only" 2000 128 \
  "${COMMON_ARGS[@]}" --time_len 16000

run_case "stage_fine_full" "convtas_source_projector_only" "calib_match_fine_full_source_projector_only" 2000 256 \
  "${COMMON_ARGS[@]}" --time_len 16000

run_case "stage_fine_full" "convtas_decoder_pre_only" "calib_match_fine_full_decoder_pre_only" 2000 512 \
  "${COMMON_ARGS[@]}" --time_len 16000

run_case "stage_fine_full" "convtas_decoder_only" "calib_match_fine_full_decoder_only" 2000 512 \
  "${COMMON_ARGS[@]}" --time_len 16000

echo "[DONE] Fine submodule HEF calib-match run_ts=${RUN_TS}"
