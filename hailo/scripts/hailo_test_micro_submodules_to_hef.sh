...
SUMMARY="${RUN_DIR}/micro_submodule_hef_summary.tsv"
if [[ ! -f "$SUMMARY" ]]; then
...
# source projector split: 256 -> 256 each
run_case "stage_micro_source_projector" "convtas_source_projector_out0" "micro_source_projector_out0" 2000 256 \
  "${COMMON_ARGS[@]}"
run_case "stage_micro_source_projector" "convtas_source_projector_out1" "micro_source_projector_out1" 2000 256 \
  "${COMMON_ARGS[@]}"

# decoder pre split: 512 -> 256 each
run_case "stage_micro_decoder_pre" "convtas_decoder_pre_half0" "micro_decoder_pre_half0" 2000 512 \
  "${COMMON_ARGS[@]}"
run_case "stage_micro_decoder_pre" "convtas_decoder_pre_half1" "micro_decoder_pre_half1" 2000 512 \
  "${COMMON_ARGS[@]}"

# decoder head split: 512 -> 1 each
run_case "stage_micro_decoder_head" "convtas_decoder_head_src0" "micro_decoder_head_src0" 2000 512 \
  "${COMMON_ARGS[@]}"
run_case "stage_micro_decoder_head" "convtas_decoder_head_src1" "micro_decoder_head_src1" 2000 512 \
  "${COMMON_ARGS[@]}"

echo "[DONE] Micro submodule HEF run_ts=${RUN_TS}"
